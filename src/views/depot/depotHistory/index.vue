  ...
  methods: {
        ...
        enterStorageHistory(this.infoId).then((res) => {
          ...
          for (const iterator of this.infoList.order_infos) {
            ...
            for (const iterator1 of iterator.inblound_infos) {
              if (
                (iterator1.odd_number == code ||
                  typeof code == "undefined" ||
                  code == "") &&
                (iterator1.godown_name == person ||
    ...
    enterStorage(searchInfo) {
      ...
      for (const index in this.addObj.order_infos) {
        ...
        if (
          this.addObj.order_infos[index].not_in_number < this.perNumber[index]
        ) {
          this.$message({
            message: "入库数过多",
            type: "error",
    ...
    fetchData(searchInfo) {
      ...
      getEnterStorage(searchInfo, this.page_size, this.page_current).then(
        (res) => {
    ...
    editSend(scope, item) {
      ...
      let data = [
        { inblound_message_id: item.id, number: this.perInfo.number },
      ];
      this.listLoading = true;
